...
def upsample(in_channels, out_channels, upscale, kernel_size=3):
    ...
    layers = []
    for i in range(int(math.log(upscale, 2))):#取 log upscale的个数
        layers.append(PixelShuffle(out_channels, scale=2)) #每次都放大两倍
    return nn.Sequential(*layers)

def classifier(in_channels, out_channels, upscale, kernel_size=3):
    # A series of x 2 upsamling until we get to the upscale we want
    layers = []
    ...
    layers.append(conv1x1)#先改变通道的数目，然后再改变图像的大小
    return nn.Sequential(*layers)

class MainDecoder(nn.Module):
    def __init__(self, upscale, conv_in_ch, num_classes):
        super(MainDecoder, self).__init__()
        # conv_in_ch 为解码器输入的通道数目
        
        self.classifier = classifier(conv_in_ch, num_classes, upscale=upscale)
        self.upsample = upsample(conv_in_ch, num_classes, upscale=upscale)#从编码器到解码器，三个子像素卷积

    def forward(self, x,probb=False):
        #这里需要一个表示
...
def get_r_adv(x, classifier,decoder, it=1, xi=1e-1, eps=10.0):
    """
    Virtual Adversarial Training
    https://arxiv.org/abs/1704.03976
    """
    x_detached = x.detach() # 512维度
    with torch.no_grad():
        k = classifier(x_detached)
        pred = F.softmax(decoder(k), dim=1)
...
class VATDecoder(nn.Module):  #这里面是进行VAT的扰动
    ...
    def forward(self, x, _):
        r_adv = get_r_adv(x, self.classifier,self.upsample, self.it, self.xi, self.eps)
...
def guided_masking(x, output, upscale, resize, return_msk_context=True):
    if len(output.shape) == 3:
        ...
        masks_context = (output.argmax(1) > 0).float().unsqueeze(1) #筛选出类别大于0的类
...
class ObjectMaskingDecoder(nn.Module):
    ...
    def forward(self, x, pred=None):
        ...
        x_masked_obj = self.classifier(x_masked_obj)
