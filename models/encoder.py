...
class _PSPModule(nn.Module): #PSP金字塔空间池化模块
    ...
    def forward(self, features):
        ...
        output = self.bottleneck(torch.cat(pyramids, dim=1)) #按照通道数进行拼接
        return output


class Encoder(nn.Module): #这里是编码器的模块
    def __init__(self, pretrained):
        ...
        model = ResNetBackbone(backbone='deepbase_resnet50_dilated8', pretrained=pretrained) #这个模型是带有空洞卷积的ResNet50
        self.base = nn.Sequential(
            nn.Sequential(model.prefix, model.maxpool), #只需要model的prefix和model.maxpool层
            model.layer1,
            model.layer2,
            model.layer3,
            model.layer4
        )
        self.psp = _PSPModule(2048, bin_sizes=[1, 2, 3, 6]) #然后是psp模块

    def forward(self, x,select=False):
        img_size = x.size(2),x.size(3)
        x = self.base(x) # 产生的特征维度是2048,resnet的最后一个维度
        feats = self.psp(x) #产生的维度是512维度
        return feats

    def get_backbone_params(self): #返回的是主干网络的参数
        return self.base.parameters()

    def get_module_params(self):#返回的是PspNet的参数
