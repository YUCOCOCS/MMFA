...
def multi_scale_predict(model, image, scales, num_classes, flip=True):
    ...
    for scale in scales:
        ...
        scaled_prediction = upsample(model(scaled_img,mode="val"))

        if flip:
            fliped_img = scaled_img.flip(-1)
            fliped_predictions = upsample(model(fliped_img,mode="val"))
...
def parse_arguments():
    ...
    parser.add_argument( '--model', default='saved/CCT/best_model.pth', type=str,
                        help='Path to the trained .pth model')    #### 这里面存储模型参数
    parser.add_argument( '--save', action='store_true', help='Save images')
    parser.add_argument('--images', default="ans", type=str,
                        help='Test images for Pascal VOC')  #### 这里面存储的是数据
