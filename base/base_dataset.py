...
class BaseDataSet(Dataset):
    def __init__(self, data_dir, split, mean, std, batch_size,ignore_index, base_size=None, augment=True, val=False,
        ...
        self.root = data_dir
        self.base_size = batch_size
        self.split = split
        self.mean = mean
        self.std = std
        self.augment = augment
        self.crop_size = crop_size # 320
        self.jitter = jitter # False
        self.image_padding = (np.array(mean)*255.).tolist()
        self.ignore_index = ignore_index  # 255
        self.return_id = return_id # False
        self.n_labeled_examples = n_labeled_examples # 1464
        self.val = val

        self.use_weak_lables = use_weak_lables # Ture
        self.weak_labels_output = weak_labels_output  #产生的弱标签的输出路径

        if self.augment:
            self.base_size = base_size #400
            self.scale = scale  #true
            self.flip = flip    #true
            self.rotate = rotate #false
            self.blur = blur  #false

        self.jitter_tf = transforms.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.1)
        self.to_tensor = transforms.ToTensor()  #转换成tensor
    ...
    def _flip(self, image, label):
        ...
        if random.random() > 0.5:
            image = np.fliplr(image).copy() #水平方向上进行旋转
            label = np.fliplr(label).copy()
        return image, label

    def _resize(self, image, label, bigger_side_to_base_size=True): # 这里可能会导致错误
    ...
    def _augmentation(self, image, label):
        ...
        if self.crop_size is not None:
            image, label = self._crop(image, label) #裁剪成固定尺寸
    ...
    def __getitem__(self, index):
        image, label, image_id =  self._load_data(index)  #第一个是图片，第二个是标签，第三个是图片的id
        if self.val: #如果是验证，则进行验证的数据增强
            image, label = self._val_augmentation(image, label)
        elif self.augment: #训练时的各种预数据处理操作
            image, label = self._augmentation(image, label)

        label = torch.from_numpy(np.array(label, dtype=np.int32)).long()
        return image, label #得到经过预处理之后的数据
